...
2. **Für jede offene URL**:
   ...
   b. **Stellenanzeige extrahieren** mit `node career-browser.mjs jd "<url>" --json` → WebFetch → WebSearch
...
1. **Repo-lokales Playwright (bevorzugt):** `node career-browser.mjs jd "<url>" --json`. Funktioniert mit SPAs und liefert strukturierte Ausgabe für den Agenten.
