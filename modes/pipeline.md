...
2. **Para cada URL pendiente**:
   ...
   b. **Extraer JD** usando `node career-browser.mjs jd "<url>" --json` → WebFetch → WebSearch
...
1. **Playwright repo-local (preferido):** `node career-browser.mjs jd "<url>" --json`. Funciona con SPAs y deja una salida estructurada que cualquier agente puede parsear.
